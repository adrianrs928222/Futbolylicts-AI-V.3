export const FUTBOLYLICTS_RULES = {
  ...
  // 1.25–1.75 sigue siendo el rango ideal. Hasta @2.50 puede entrar si el
  // mercado sigue siendo ALTA/MUY ALTA y ayuda a construir mejor la combinada.
  eligibleOddsMax: 2.50,
  bttsMinOdds: 1.35,
  ...
  maxFixturesForCombo: 150,
  comboBeamWidth: 40000,
  ...
  globalDeepScanFixtures: Number(process.env.GLOBAL_DAY_DEEP_SCAN_FIXTURES ?? 150),
  globalMaxPerLeague: Number(process.env.GLOBAL_DAY_MAX_PER_LEAGUE ?? 20),
  globalStandingLeagues: Number(process.env.GLOBAL_DAY_STANDING_LEAGUES ?? 12),
  globalOtherLeagueSlots: 0,

  // Alias histórico para despliegues previos.
  maxFixturesToEnrich: Number(process.env.MAX_FIXTURES_TO_ENRICH ?? 150),

  excludedByDefault: {
    // Femenino sigue excluido salvo UEFA Women’s Champions League, UEFA Women’s Europa Cup y Liga F española.
    women: true,
    youth: true,
    reserves: true,
    friendlies: true,
...
export const API_POLICY = {
  dailyCallBudget: Number(process.env.API_CALL_BUDGET_DAILY ?? 300),
  ...
  analysisTtlSeconds: Math.max(60 * 60, Number(process.env.CACHE_ANALYSIS_TTL_SECONDS ?? 6 * 60 * 60)),
