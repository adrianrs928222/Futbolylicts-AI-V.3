...
export function eligibleCandidates(candidates: MarketCandidate[]): MarketCandidate[] {
  return candidates.filter((candidate) => {
    ...
    // v0.19: ALTA y MUY ALTA son válidas. MUY ALTA ya no se excluye.
...
export function buildBestCombo(date: string, allCandidates: MarketCandidate[]): DailyCombo {
  ...
  // v0.25 MAX: @8–@10 es OBJETIVO, no una cifra que se fuerce. Si no existe
  // con picks ALTA/MUY ALTA, publicamos la mejor combinación real del modelo y
  // mostramos su producto exacto de cuotas EST. (puede ser @2, @3, @5...).
  const fallbackPool = searchCombinationStates(hardFixtureOptions, FUTBOLYLICTS_RULES.emergencyMaxSelections)
    .filter((state) => state.picks.length >= FUTBOLYLICTS_RULES.minSelections)
    .sort((a, b) => finalUtility(b) - finalUtility(a));
  const fallback = fallbackPool[0];
  if (fallback) {
    const refined = refineChosenState(fallback, eligibleRaw);
    const chosen = finalUtility(refined) >= finalUtility(fallback) ? refined : fallback;
    const result = toDailyCombo(date, chosen, true);
    result.message = `Mejor Combinada del Día disponible: @${round2(chosen.totalOdds).toFixed(2)} EST. El objetivo sigue siendo @8–@10, pero hoy no se fuerza ni se inventa la cuota. Total = multiplicación exacta de las cuotas EST. de cada pata.`;
    return result;
  }
  ...
  return {
    date, picks: [], totalOdds: 0, globalScore: 0, globalProbability: 0,
    globalConfidence: "BAJA", targetReached: false, official: false,
    message: "No hay suficientes picks ALTA/MUY ALTA para construir una combinada fiable. El motor no añade selecciones flojas solo para alcanzar @8–@10.",
  };
}
