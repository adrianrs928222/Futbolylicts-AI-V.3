...
function isVeryHigh(candidate: MarketCandidate): boolean {
  return (
    candidate.score >= FUTBOLYLICTS_RULES.veryHighScore &&
    candidate.probability >= FUTBOLYLICTS_RULES.veryHighProbability
  );
}
...
export function eligibleCandidates(candidates: MarketCandidate[]): MarketCandidate[] {
  return candidates.filter((candidate) => {
    ...
    if (FUTBOLYLICTS_RULES.comboOnlyHighConfidence && isVeryHigh(candidate)) return false;
...
function candidateUtility(candidate: MarketCandidate): number {
  ...
  // BTTS compite exactamente igual que el resto: puede repetirse si sigue siendo ALTA.
...
export function buildBestCombo(
  ...
  if (!chosen) {
    ...
    if (partial) {
      ...
      return {
        ...
        targetReached: false,
        official: false,
        message: `Hay ${partial.picks.length} pick${partial.picks.length === 1 ? "" : "s"} que pasan todos los filtros. Aún faltan partidos válidos para cerrar una combinada cercana a @10.`,
      };
    }
  ...
  return {
    ...
    message: insideMainTarget
      ? "¡Vamos con confianza! 🍀"
      : chosen.totalOdds < FUTBOLYLICTS_RULES.targetTotalOddsMin
        ? `Combinada válida por calidad (@${round2(chosen.totalOdds).toFixed(2)}), aunque hoy no alcanza el objetivo alrededor de @10 sin añadir riesgo.`
        : `Combinada válida @${round2(chosen.totalOdds).toFixed(2)}; supera ligeramente la zona objetivo alrededor de @10.`,
  };
}
