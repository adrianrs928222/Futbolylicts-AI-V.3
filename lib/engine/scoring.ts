...
export function estimatedModelOdds(probability: number): number {
  // Cuota justa orientativa derivada SOLO de la probabilidad del motor.
  // No intenta imitar Bet365 ni ningún bookmaker y nunca se marca como REAL.
  // Ejemplos: 80% -> @1.25, 78% -> @1.28, 70% -> @1.43.
  const p = clamp(probability, 0.05, 0.95);
  const fair = 1 / p;
  return Math.round(clamp(fair, 1.05, 20) * 100) / 100;
}
