...
export function estimatedModelOdds(probability: number, market: MarketKey = "OVER_1_5"): number {
  // v0.25 MAX: cuota JUSTA/ESTIMADA derivada directamente de la probabilidad.
  // No se empuja artificialmente hacia 1.50–2.00 para poder alcanzar @8–@10.
  // Aplicamos solo un pequeño margen teórico de bookmaker; por tanto una selección
  // del 80% ronda @1.19, no @1.50. Sigue siendo EST., nunca una cuota real.
  const p = clamp(probability, 0.05, 0.95);
  const { overround } = oddsCalibration(market);
  const impliedWithMargin = clamp(p * overround, 0.07, 0.96);
  const decimal = 1 / impliedWithMargin;
  return Math.round(clamp(decimal, 1.04, 12) * 100) / 100;
}
...
function hasOver25Condition(market: MarketKey) {
  return ["OVER_2_5", "COMBO_1X_OVER_2_5", "COMBO_X2_OVER_2_5", "COMBO_12_OVER_2_5", "COMBO_HOME_WIN_OVER_2_5", "COMBO_AWAY_WIN_OVER_2_5"].includes(market);
}
...
export function candidateIntelligence(candidate: MarketCandidate): number {
  const complexityPenalty = isSameGameComboMarket(candidate.market) ? 0.24 : 0;
  return candidate.score * 12 + candidate.probability * 8 + oddsUsefulness(candidate.odds) * 1.1 + Math.log(candidate.odds) * 0.35 - complexityPenalty;
}
...
const WOMENS_EUROPA_CUP_MARKETS: MarketKey[] = [
  "DOUBLE_CHANCE_1X",
  "DOUBLE_CHANCE_X2",
  "DOUBLE_CHANCE_12",
  "OVER_2_5",
...
function isWomensEuropaCupFixture(fixture: EnrichedFixture): boolean {
  const league = fixture.fixture.league.name
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase();
  return /europa cup/.test(league) && /women|womens|woman|female|femenin/.test(league);
}
...
export function scoreMarkets(fixture: EnrichedFixture): MarketCandidate[] {
  // UEFA Women's Europa Cup: la casa suele ofrecer de forma consistente O/U 2.5
  // y doble oportunidad. Para esta competición no generamos BTTS, +1.5, ganador
  // ni combinados sintéticos: el motor compara únicamente +2.5 y 1X/X2/12.
  const analysisMarkets = isWomensEuropaCupFixture(fixture)
    ? WOMENS_EUROPA_CUP_MARKETS
    : ANALYSIS_MARKETS;

  const candidates: MarketCandidate[] = analysisMarkets.map((market) => {
    const probability = clamp(marketProbability(market, fixture), 0.01, 0.99);
  ...
  const isUseful = (candidate: MarketCandidate) =>
  ...
  const top = ranked.find(isUseful) ?? ranked[0];
  if (top) {
...
export function rankMarketsForPricing(fixture: EnrichedFixture): MarketKey[] {
  const pricingMarkets = isWomensEuropaCupFixture(fixture)
    ? WOMENS_EUROPA_CUP_MARKETS
    : PRICING_MARKETS;
