...
function buildAnalyzedFixtureSummaries(
  ...
  return enriched.map<AnalyzedFixtureSummary>((item) => {
    ...
    // v0.16: para la combinada visual no nos quedamos automáticamente con el
    // mercado de porcentaje más alto si su cuota justa es demasiado pequeña.
    // Primero buscamos un mercado que siga siendo ALTA/MUY ALTA y que además
    // tenga cuota útil (REAL o EST.) desde @1.25. Así +2.5, BTTS, X2, 12 o los
    // combinados pueden ganar frente a un +1.5 al 94% pero @1.06 EST.
    const marketSnapshot = rankedAnalysisMarkets.map((market) => {
      const priced = pricedCandidates.find(
        (candidate) => candidate.market === market && candidate.realOdds,
      );
      const probability = priced?.probability ?? marketProbability(market, item);
      const score = priced?.score ?? statisticalMarketScore(market, item);
      const odds = priced?.odds ?? estimatedModelOdds(probability);
      const realOdds = Boolean(priced?.realOdds);
      const usefulOdds =
        odds >= FUTBOLYLICTS_RULES.preferredOddsMin &&
        odds <= FUTBOLYLICTS_RULES.preferredOddsMax &&
        (market !== "BTTS_YES" || odds >= FUTBOLYLICTS_RULES.bttsMinOdds);
      const highProfile =
        probability >= FUTBOLYLICTS_RULES.minProbability &&
        score >= FUTBOLYLICTS_RULES.minScore;
      return { market, probability, score, odds, realOdds, usefulOdds, highProfile };
    });

    const preferred = marketSnapshot.find((entry) => entry.highProfile && entry.usefulOdds);
    const fallback = marketSnapshot[0];
    const selected = preferred ?? fallback;
    const bestMarket = selected?.market;

    if (!bestMarket || !selected) {
      return {
        ...
        realOdds: false,
        status: "FUERA",
        explanation: "El motor no encontró un mercado con suficiente señal estadística para este partido.",
        alternatives: [],
      };
    }
    ...
    const realOdds = selected.realOdds;

    const alternatives = marketSnapshot
      .filter((entry) => entry.market !== bestMarket)
      .slice(0, 4)
      .map((entry) => ({
    ...
    if (!realOdds) {
      ...
      return {
        ...
        leagueName: item.fixture.league.name,
        category: item.category,
        bestMarketLabel: MARKET_LABELS[bestMarket] ?? bestMarket,
        probability,
        score,
        odds,
        realOdds: false,
        status: "SIN_CUOTA",
        explanation: highProfile
          ? `Perfil ALTA por datos (${Math.round(probability * 100)}% · ${score.toFixed(1)}/10). Cuota justa estimada por el modelo: @${odds.toFixed(2)} EST.; falta la cuota real para comprobar el valor de mercado.`
          : `Cuota justa estimada del modelo: @${odds.toFixed(2)} EST. El partido sigue visible, pero no supera a la vez probabilidad y nota para ser ALTA.`,
        alternatives,
      };
    }

    const failures: string[] = [];
    if (probability < FUTBOLYLICTS_RULES.minProbability) {
      failures.push(`probabilidad ${Math.round(probability * 100)}% (<70%)`);
    }
    if (score < FUTBOLYLICTS_RULES.minScore) {
      failures.push(`nota ${score.toFixed(1)}/10 (<8.0)`);
    }
    if (odds < FUTBOLYLICTS_RULES.preferredOddsMin || odds > FUTBOLYLICTS_RULES.preferredOddsMax) {
      failures.push(`cuota @${odds.toFixed(2)} fuera de @1.25–@1.75`);
    }
    const implied = 1 / Math.max(odds, 1.01);
    const edge = probability - implied;
    if (edge < FUTBOLYLICTS_RULES.minValueEdge) {
      failures.push(`valor insuficiente frente a la cuota (${Math.round(edge * 100)} pts)`);
    }

    const passes = failures.length === 0;
    const near = !passes && probability >= 0.67 && score >= 7.7;
    return {
      fixtureId: item.fixture.id,
      fixtureLabel: `${item.fixture.home.name} – ${item.fixture.away.name}`,
      leagueName: item.fixture.league.name,
      ...
      explanation: passes
        ? "Pasa probabilidad, nota, rango de cuota y valor frente a la cuota REAL."
