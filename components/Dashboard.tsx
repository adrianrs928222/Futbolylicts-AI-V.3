...
export default function Dashboard({ analysis }: { analysis: DailyAnalysis }) {
  ...
  const [analyzedDay, setAnalyzedDay] = useState<"today" | "tomorrow" | "all">("today");
  ...
  const filteredAnalyzedMenu = useMemo(() => {
    ...
    const madridDayKey = (value: string | number | Date) => new Intl.DateTimeFormat("en-CA", { timeZone: "Europe/Madrid", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date(value));
    const todayKey = madridDayKey(new Date());
    const tomorrow = new Date(); tomorrow.setDate(tomorrow.getDate() + 1);
    const tomorrowKey = madridDayKey(tomorrow);
    return visibleAnalyzed.filter((i) => {
      const meta = analyzedFixtureMeta.get(i.fixtureId);
      const fixtureDay = meta?.kickoff ? madridDayKey(meta.kickoff) : "";
      const matchesDay = analyzedDay === "all" || (analyzedDay === "today" ? fixtureDay === todayKey : fixtureDay === tomorrowKey);
      ...
      const am = analyzedFixtureMeta.get(a.fixtureId); const bm = analyzedFixtureMeta.get(b.fixtureId);
      return (am?.timestamp ?? Number.MAX_SAFE_INTEGER) - (bm?.timestamp ?? Number.MAX_SAFE_INTEGER);
    });
  }, [visibleAnalyzed, analyzedFilter, analyzedDay, analyzedQuery, analyzedFixtureMeta]);
    ...
    <section id="analizados" className="panel analyzed-explorer">
      <div className="section-title-row">
        <div>
          ...
          <p>El motor analiza el máximo que permite la fuente de datos. Por defecto muestra los partidos de HOY. Puedes cambiar a Mañana o Todos; dentro de cada vista se ordenan por hora para que los encuentros de hoy no queden desplazados.</p>
        </div>
      ...
      <div className="analyzed-toolbar">
        ...
        <div className="analyzed-tabs analyzed-day-tabs" role="tablist" aria-label="Filtrar analizados por día">
          <button className={analyzedDay === "today" ? "active" : ""} onClick={() => { setAnalyzedDay("today"); setAnalyzedVisibleCount(12); }}>Hoy</button>
          <button className={analyzedDay === "tomorrow" ? "active" : ""} onClick={() => { setAnalyzedDay("tomorrow"); setAnalyzedVisibleCount(12); }}>Mañana</button>
          <button className={analyzedDay === "all" ? "active" : ""} onClick={() => { setAnalyzedDay("all"); setAnalyzedVisibleCount(12); }}>Todos</button>
        </div>
